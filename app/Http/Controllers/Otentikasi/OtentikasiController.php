    ...
    public function cek_login(Request $request)
    {
        ...
        if (isset($request->user_login)) {
            $logged_in = Otentikasi::where('user_login', $request->user_login)
                ->first();
            if ($logged_in <> null) {
                if (Hash::check($request->password, $logged_in->password)) {
                    if ($logged_in->is_disabled == 0) {
                        $roleId = DB::table('user_roles')->where('user_id', '=', $logged_in->id)->first();
                        session([
                            'LAST_ACTIVITY' => time(), 'logged_in' => true, 'user_id' => $logged_in->id, 'nama' => $logged_in->nama, 'user_login' => $logged_in->user_login,
                            'email' => $logged_in->email, 'level' => $logged_in->level, 'password' => $request->password, 'role_id' => $roleId->role_id, 'divisi_id' => $logged_in->divisi_id
                        ]);
                        ...
                        return redirect('/login')->with('status', 'Login Failed, Akun sudah tidak aktif');
                    }
                }
            }
        }
