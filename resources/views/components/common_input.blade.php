...
<div class="item form-group">
    ...
    <div class="col-md-{{$input_width1}} col-sm-{{$input_width1}} col-xs-{{$input_width2}}">
        <input type="{{$data_type}}" placeholder="{{$placeholder ?? ''}}" id="{{$input_id}}" name="{{$input_id}}" class="form-control @error($input_id) is-invalid @enderror " value="@if($for_create_edit=='create'){{old($input_id)}} @else{{$detail_table->$input_id}}@endif" @isset($other_option) {{$other_option }} @endisset>
        @error($input_id)
